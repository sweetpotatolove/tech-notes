...
- 운영성(Operability)
    - 운영팀이 시스템을 **쉽게 관리하고 유지보수**할 수 있도록 설계하여, 서비스가 중단 없이 안정적으로 운영되도록 해야함
    - 효율적인 운영이 매우 중요함
    ...
    - 데이터 시스템의 운영성 향상 (Enhancing Operability in Data Systems)
        ...
        - 특정 머신에 의존하지 않는 구조
        - 충분하고 체계적인 문서화
        - 유연한 설정(Configuration)
        - 자가 복구(Self-Healing) 기능
        - 예측 가능한 동작

- 단순성(Simplicity)
    - 새로운 개발자도 쉽게 이해할 수 있도록 시스템 내부의 **불필요한 복잡성을 제거**해야함
        - 이는 사용자 인터페이스(UI)가 아니라 시스템 내부 구조를 단순하게 만드는 것을 의미

    - 규모가 커지는 소프트웨어 프로젝트의 복잡성 (Complexity in Growing Software Projects)
        - 프로젝트 규모가 커질수록 시스템은 더욱 복잡해지고 이해하기 어려워짐
        - 복잡성이 증가하면 개발 속도가 느려지고 유지보수 비용도 증가함
        - 이처럼 지나치게 복잡한 프로젝트를 'Big Ball of Mud'라고 부름

    - 복잡성이 유지보수에 미치는 영향 (Impact of Complexity on Maintenance)
        ...
        - 복잡성을 줄이면 유지보수성이 향상됨
        - 따라서, 시스템 개발에서 단순성은 중요한 목표가 되어야함
    
    - 복잡성을 줄이기 위한 추상화 (Abstraction as a Tool to Reduce Complexity)
        - 단순하고 이해하기 쉬운 인터페이스 뒤에 구현 세부 사항을 숨기는 것
        - 여러 애플리케이션에서 코드를 재사용할 수 있도록 함
        - 개선 사항이 이를 사용하는 모든 애플리케이션에 적용되므로 소프트웨어 품질이 향상됨
    
    - 좋은 추상화를 찾는 어려움 (Challenges in Finding Good Abstractions)
        - 특히 분산 시스템에서는 효과적인 추상화를 찾기가 어려움
        - 알고리즘을 유용한 추상화 형태로 구성하는 일도 쉽지 않음
...
- 변화 용이성(Evolvability)
    - 앞으로 새로운 요구사항과 예상하지 못한 사용 사례가 등장하더라도 개발자가 **쉽게 수정하고 확장**할 수 있도록 시스템을 설계해야함
        - 이를 확장성(Extensibility), 수정 용이성(Modifiability), 또는 유연성(Plasticity)이라고도 함
    
    - 시스템 요구사항은 반드시 변화한다 (Inevitable Change in System Requirements)
        - 시스템 요구사항은 영원히 변하지 않는 경우가 거의 없음
        - 변화를 일으키는 요인
            - 새로운 정보
            - 예상하지 못한 사용 사례
            - 비즈니스 우선순위 변경
            ...
            - 플랫폼 업데이트
            - 법률 및 규제 변화
            - 시스템 성장에 따른 아키텍처 변경 등

    - 변화에 대응하기 위한 애자일 방법론 (Agile Methodologies for Adaptation)
        - 애자일 개발 방식은 변화하는 요구사항에 적응할 수 있는 개발 프레임워크를 제공함
        - 테스트 주도 개발(TDD)과 리팩터링 같은 기법은 빠르게 변화하는 환경에서의 개발을 지원함
        - 이러한 애자일 기법은 일반적으로 개별 애플리케이션 내부의 작은 변경에 초점을 맞춤

    - 대규모 시스템으로 애자일 확장하기 (Scaling Agility to Larger Systems)
        - 여러 애플리케이션과 서비스로 구성된 대규모 데이터 시스템에서도 애자일한 개발 방식을 적용할 필요가 있음

    - 단순성과 추상화를 통한 변화 용이성 확보 (Evolvability Through Simplicity and Abstraction)
        - 데이터 시스템을 얼마나 쉽게 수정할 수 있는지는 시스템의 단순성과 추상화의 품질에 크게 좌우됨
        - 단순하고 이해하기 쉬운 시스템은 복잡한 시스템보다 새로운 요구사항에 더 쉽게 적응할 수 있음
        - 변화 용이성(Evolvability)은 데이터 시스템 수준에서의 민첩성을 의미하며, 시간이 지나면서 지속적으로 발전할 수 있는 시스템을 설계하는 것이 중요함


### 정리
- 3 concerns in the most software systems
    ...
    - Maintainability
        - Operability: Simplifying Operations
        - Simplicity: Handling Complexity
        - Evolvability: Simplifying Change
